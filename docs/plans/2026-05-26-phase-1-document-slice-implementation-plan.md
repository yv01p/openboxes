...
| T5-I1 | Important | Pre-Task 9 (before nginx exposes externally) | springdoc OpenAPI schema collapses the three `params=`-discriminated GETs into one operation with all three params marked `required: true`, and the `DocumentCode` enum is not emitted in `components.schemas` — misleading for external consumers reading `/v3/api-docs` | `controller/DocumentController.java:76-103` |
| T5-I2 | Important | Any time | `POST /api/documents` returns 200, not 201 Created + `Location` header. REST convention violation; tools like Postman/OpenAPI generators will flag. Three-line fix using `ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")` | `controller/DocumentController.java:105-117` |
| T5-I3 | Important | Bundle with Task 7 | `SecurityConfig` does not exclude `UserDetailsServiceAutoConfiguration`; startup log emits "Using generated security password: <uuid>" WARN even though `permitAll()` short-circuits the chain. Cosmetic now, footgun if anyone configures against the printed password. Task 7 replaces this whole config anyway | `security/SecurityConfig.java` (Task 7) |
