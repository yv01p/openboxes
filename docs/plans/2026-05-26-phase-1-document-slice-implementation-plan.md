...
| T7-M1 | Minor | Task 11 hardening | Silent JWT-rejection logging gap: `catch (JwtException ignored)` swallows every invalid/tampered/expired token without a log entry. Add `logger.debug("JWT cookie rejected: {}", e.getClass().getSimpleName())` (class name only — Jwts messages can include token fragments). Current behavior matches spec ("downstream rejects" via authenticated() → 401) but a debug log aids incident response | `services/document-service/src/main/java/org/openboxes/document/security/JwtCookieAuthFilter.java:53` |
...
| T8b-I1 | Important | ACTIVE — Task 9 has landed, this now affects rendered pages | Add `size` (byte length) to document-service Document JSON. `grails-app/views/order/_orderDocuments.gsp:23,46` renders `${documentInstance?.size} bytes` / `${documentTemplate?.size} bytes`; the Map from `documentClient.fetchById`/`findByCode` has no `size` key (Document entity in document-service doesn't expose it). Computed-server-side option: add `@Transient Long getSize() { return fileContents != null ? fileContents.length : null; }` to `Document.java`. Post-Task-9 the document-template list renders "null bytes" without this | `services/document-service/src/main/java/org/openboxes/document/entity/Document.java` |
| T8b-I2 | Important | ACTIVE — Task 9 has landed, document-type column renders raw Map.toString() | `format:metadata` taglib silently degrades for Map input: `grails-app/taglib/.../FormatTagLib.groovy:212` calls `attrs.obj.properties.get("name")` which on a `Map` returns Groovy class properties (not Map keys), falls through to `Map.toString()` rendering `[id:..., name:..., ...]` instead of the type name. Visible at `grails-app/views/order/_orderDocuments.gsp:21,44` passing `documentInstance?.documentType` to `<format:metadata>`. Fix in taglib (treat Maps as bag-of-attributes) OR rewrite GSP to read `.documentType?.name` directly | `grails-app/taglib/org/pih/warehouse/FormatTagLib.groovy:212` |
| T8b-I3 | Important | Pre-Task 11 hardening | `DocumentClient`: drain error streams + call `disconnect()` on non-2xx HttpURLConnection paths to prevent socket leak under sustained failure. Per Java `HttpURLConnection` contract, the underlying socket is not returned to keep-alive pool until error stream is fully drained. Affects ~7 throw sites: lines 47, 53, 59, 70, 77, 83, 110. Pattern: `try { conn.errorStream?.bytes } finally { conn.disconnect() }` before throw. Not blocking pre-Task 9 (zero traffic) but matters at sustained 401/5xx load | `grails-app/services/org/pih/warehouse/core/DocumentClient.groovy:47,53,59,70,77,83,110` |
| T8b-I4 | Important | Pre-Task 11 hardening | `DocumentClient.create()` instantiates `new RestTemplate()` with default `SimpleClientHttpRequestFactory` — NO `connectTimeout` or `readTimeout`. A slow/hung document-service will block upload threads indefinitely. Mirror the 5s/30s from `openConn()`. Bundle with T8b-M3 (hoist RestTemplate to bean field) | `grails-app/services/org/pih/warehouse/core/DocumentClient.groovy:97-105` |
| T8b-I5 | Important | Pre-Task 11 hardening | `DocumentUploadController.upload` NPE risk: if `command.file` is null (missing multipart, wrong field name) `documentClient.create(null, null, null, null)` is invoked — `ByteArrayResource(null)` throws. Also no validator (`nullable: false`) on `MultipartFile file`. Field rename from `document` to `file` is a contract break for any caller still using the old field name — document or restore. Add null guard + `@BindUsing` or explicit constraint | `grails-app/controllers/org/pih/warehouse/shipping/DocumentUploadController.groovy:26-31` |
...
| T8b-M3 | Minor | Bundle with T8b-I4 | Hoist `RestTemplate` to a per-bean field in `DocumentClient` (initialize in `@PostConstruct` or via `@Bean`); eliminate per-call instantiation in `create()`. Thread-safe so not a correctness issue but wasteful under load | `grails-app/services/org/pih/warehouse/core/DocumentClient.groovy:97` |
...
| T8b-M5 | Minor | Any time hygiene | Add `// TODO` marker to `InvoiceController.groovy:168` `Document.load(documentInstance.id)` site for consistency with the 4 other `Document.load(id)` bridge sites (all of which carry TODO markers naming Phase 2+ as the dissolution point) | `grails-app/controllers/org/pih/warehouse/invoice/InvoiceController.groovy:168` |
...
| T8b-M7 | Minor | Phase 2 (when migration cookie context resolved) | `DocumentClient.currentObxToken()` returns null in background contexts (BootStrap, scheduled jobs, Quartz). `MigrationService.createMigrationReport()` runs in BootStrap context and now calls `documentClient.create(...)` without a cookie — document-service JWT filter will return 401. Either: (a) issue a service-to-service JWT for background callers, (b) skip migration-report side-effect in BootStrap, (c) keep Grails-side direct DB write for this specific call. **Now ACTIVE post-Task 9** | `grails-app/services/org/pih/warehouse/data/MigrationService.groovy:1192`, `grails-app/services/org/pih/warehouse/core/DocumentClient.groovy:21-30` |
| T9-M1 | Minor | Task 11 hardening | nginx `client_max_body_size` is unset — defaults to 1MB. Spring Boot multipart in document-service is configured to 10MB (per `application.yml` mirroring Grails-side `Document.fileContents` constraint). Uploads between 1MB and 10MB will be rejected by nginx with `413 Request Entity Too Large` before reaching document-service. Add `client_max_body_size 10m;` (or higher to match the original Grails constraint) at server-block or `/api/documents` location-block scope. Bundle with Task 11 hardening | `docker/nginx/conf.d/app.conf` |
