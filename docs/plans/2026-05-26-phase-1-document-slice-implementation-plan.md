...
| T4-I2 | Important | Task 5 | `getAllDocumentTypes()` has zero callers in Grails AND no Task 5 controller mapping — dead surface area. Either delete the service method (YAGNI) or add `Sort.by("sortOrder")` to match Grails `DocumentType` default `mapping = { sort "sortOrder" }` if a controller will be wired | `service/DocumentService.java:76` |
| T4-M1 | Minor | Task 5 | `delete(id)` throws `EmptyResultDataAccessException` on missing row; controller must add an `@ExceptionHandler` mapping it to HTTP 404 or accept Spring's default 500 | `controller/DocumentController.java` (Task 5) |
...
| T4-M3 | Minor | Task 5 | `create()` silently drops unknown `documentTypeId` (lookup-miss results in `documentType = null`). Decide for REST surface: 400 on bad typeId (better UX) vs silent-drop (preserves Grails behavior). Decision should land in Task 5 commit notes | `service/DocumentService.java:97-99` |
