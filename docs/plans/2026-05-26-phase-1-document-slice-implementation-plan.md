...
|---|---|---|---|---|
| T3-M2 | Minor | Any time | Guard `findByDocumentType_IdIn` against null/empty list in service layer (some DBs balk on `IN ()`; Hibernate 6 + MariaDB handles, but explicit guard is portable) | `service/DocumentService.java` |
| T3-M6 | Minor | Any time | Add `@Column(updatable=false)` to `dateCreated` on both entities; ensure service `create()` sets it explicitly (no `@CreationTimestamp` — would conflict with Grails-side GORM auto-stamp) | `entity/Document.java:63`, `entity/DocumentType.java:49` |
...
| T5-M6 | Minor | Any time | `ContentDisposition.attachment().filename(name, UTF_8)` always emits the encoded-word fallback even for ASCII filenames (`filename="=?UTF-8?Q?..."?="; filename*=UTF-8''...`). Cosmetic; modern browsers prefer `filename*`. Standards-compliant | `controller/DocumentController.java:63-67` |
...
| T8b-M1 | Minor | Phase X (after callers migrate) | Tighten `def document` → `Map` in 6 method signatures: `DocumentService.scaleImage` (line 91), `TemplateService.renderTemplate` (line 28), `DocumentTemplateService.{renderGroovyServerPageDocumentTemplate, renderInvoiceTemplate, renderOrderDocumentTemplate, renderRequisitionDocumentTemplate}`. Spec specified `Map`; implementer chose `def` for transitional Document↔Map coexistence. Restore static typing once Phase X dissolves the `Document.load(id)` bridge pattern | `grails-app/services/org/pih/warehouse/core/DocumentService.groovy:91`, `TemplateService.groovy:28`, `DocumentTemplateService.groovy` |
