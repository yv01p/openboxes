...
**Carry-forwards landed**: RC-16 (`/api/.../products/classifications` 500). **Carry-forwards explicitly deferred**: RC-13 remainder (CategoryApiController deletion / `runtime.groovy:757` fillRate categoryApi dependency).
...
**RC-16 lands in core**: relocate the classification query to inventory-service. `InventoryLevel.abcClass` is read locally; `facility → inventory` resolves locally via the `Inventory.warehouse` FK; the all-products distinct-`abcClass` set comes from a **new tiny read-only catalog-service endpoint** (owner-exists → HTTP, per parent §4.3). This deletes the Grails `ProductClassificationApiController` + `ProductClassificationService` — a real strangler bite, better optics than Phase 3/5's near-zero.
...
The classification feature (currently HTTP 500; `ProductClassificationApiController` → `ProductClassificationService.list(facilityId)`) is **served from inventory-service**:
...
**Deletes** Grails `ProductClassificationApiController.groovy` + `ProductClassificationService.groovy` (+ the `ProductClassificationServiceSpec` unit test; the integration-test specs port to inventory-service). Verified: the service has no caller other than its controller, so this is a clean delete.
...
| **T7** | Delete Grails `ProductClassificationApiController` + `ProductClassificationService` + unit spec; port/retire its integration specs; grep-verify no dangling refs | Real strangler bite |
...
| **T9** | Real-payload ground-truth for RC-16 (capture real React request/response; RC-43) + seeded read-through e2e through nginx (RC-45); Playwright spec(s) for the consumed GET surface | Empty-DB self-skip caveat applies — seeded round-trip mandatory |
...
- **RC-16 union behavior vs the broken Grails endpoint** — the endpoint currently 500s, so there's no working behavior to byte-match; correctness is defined by the ported logic (local InventoryLevel distinct ∪ catalog Product distinct). *Mitigation*: real-payload ground-truth (RC-43) + seeded round-trip asserting the union (RC-45).
...
| A11 | `/api/...classifications` currently 500 | ✅ (documented) | Phase 5 retro RC-16 / T12 done-gate observation; root cause uninvestigated (fix = serve from inventory-service, not patch Grails) |
