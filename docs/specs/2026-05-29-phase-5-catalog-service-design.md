...
No cross-service concern at the JPA-mapping level — both entities live in catalog-service. Grails-side `Tag.addToProducts/removeFromProducts` continues to work via shared DB (Grails writes `product_tag` rows directly via Hibernate 5); if catalog-service Tag writes are in scope per T1 audit, they hit the same table via Hibernate 6. **Empirical finding (CDR R3):** `product_tag` has NO unique constraint on `(product_id, tag_id)` (verified via `SHOW CREATE TABLE product_tag` — two single-column FK indexes only; `tag_id` is also nullable). Concurrent writes from both sides therefore produce silent duplicate rows rather than DB-level collision exceptions. **Resolution deferred to T1 audit:** if T1 finds catalog-service Tag write callers (per CDR R1 §3.1 expanded enumeration), the M:N concurrent-write protocol becomes a forced decision at T1 (options: accept-silent-duplicates / escalate-to-Phase-5.5-or-Phase-6 with explicit coordination / app-layer pair-uniqueness check in TagService pre-insert). If T1 confirms Tag is GET-only (current FD#5 default), the race cannot manifest in Phase 5 and the decision defers to whichever phase first introduces catalog-side Tag writes.
...
- **Tag↔Product M:N concurrent write coordination.** Grails Tag.addToProducts and catalog-service POST /api/tag (if T1 finds it in scope) both write `product_tag`. **Empirically NO unique-pair constraint** (CDR R3); concurrent INSERTs produce silent duplicate rows; deletes also uncoordinated. **Mitigation**: T1 audit's forced-decision protocol fires if Tag writes are in scope (per FD#9); if Tag is GET-only (CDR R1 §3.1 default), race cannot manifest in Phase 5. If writes are in scope, T9 integration tests must explicitly test the post-T1 chosen resolution (accept-duplicates / app-layer-dedup / escalate).
