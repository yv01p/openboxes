...
|---|----------|--------|-------------|----------|-------------|
| RC-1 | Important | T9 quality-reviewer | Organization @Column(nullable=false) on code/name/active diverged from production schema (NULLABLE for SINGLE_TABLE support); ddl-auto:validate didn't catch | A (codify) | **FIXED** at `8cba628f1` (Phase 4 T9 fixup); rule codified at Phase 4.1 4.1-T1 in `docs/process/sdd-reviewer-checklist.md` |
| RC-2 | Important | T8 quality-reviewer | CI workflow `.github/workflows/e2e-tests.yml` missing `:organization-service:bootJar` + health probe + log dump | A (codify) | **FIXED** at `2e25af837` (Phase 4 T11 brought-forward); rule codified at Phase 4.1 4.1-T2 in `docs/process/plan-ordering-rules.md` |
| RC-3 | Important | T10 quality-reviewer | Login helper duplicated across `organization-service.spec.ts`, `location-service.spec.ts`, `identity-caller-regression.spec.ts` | B (accumulating) | **FIXED** at Phase 4.1 4.1-T3 — `e2e/fixtures/auth.ts` extracted; 3 specs migrated (org + location + identity-caller-regression); identity-password-reset gets breadcrumb only (different password requirements per A7) |
| RC-4 | Minor | T4 quality-reviewer | PartyRole `setEndDate` setter overbuilding (1 line cosmetic) | C (1-line) | **FIXED** at Phase 4.1 4.1-T5 |
| RC-5 | Minor | T6 quality-reviewer | Unused `Party` import in `PartyService.java:4` | C (1-line) | **FIXED** at Phase 4.1 4.1-T5 |
| RC-6 | Minor | T7 quality-reviewer | POST 201 missing `Location` header (REST conformance) | D (Grails parity) | **DELETED** from backlog — intentional Grails parity per CDR R3; not pursuing |
| RC-7 | Minor | T7 quality-reviewer | 404 returns empty body (no `{data: null, error: ...}` envelope) | D (Grails parity) | **DELETED** from backlog — intentional product decision per Grails 404 behavior; not pursuing |
| RC-8 | Minor | T7 quality-reviewer | Single-letter constructor params in REST controllers | D (Grails parity) | **DELETED** from backlog — Phase 3 style; cosmetic-only; not pursuing |
| RC-9 | Minor | T7 quality-reviewer | Two classes (`OrganizationController` + `ReferenceController`) at same `@RequestMapping("/api/organization")` — readability not function | D (Grails parity) | **DELETED** from backlog — Spring AntPathMatcher resolves correctly; doc opportunity not pursued |
| RC-10 | Minor | T8 quality-reviewer | nginx block-ordering convention undeclared (currently insertion-order, not alphabetical) | C (1-line) | **FIXED** at Phase 4.1 4.1-T4 (merged into proxy_params extraction commit per A17 conflict-prevention rule) |
| RC-11 | Minor | T8 quality-reviewer | nginx `proxy_set_header` blocks duplicated across N service entries | B (accumulating) | **FIXED** at Phase 4.1 4.1-T4 — `docker/nginx/conf.d/proxy_params` extracted; 8 blocks migrated (6 Spring Boot service routes + 2 Grails-facing catch-alls per CDR R1 option-b decision) |
...
| RC-13 | Minor | T9 quality-reviewer | `partyTypeCache_refreshOnEmptyList` test name oversells coverage | C (1-line) | **FIXED** at Phase 4.1 4.1-T5 |
...
| RC-19 | Minor | T10 quality-reviewer | `request: any` instead of `APIRequestContext` (verbatim from CIR-R3-approved plan; all existing specs use `any`) | F (defensible) | **DELETED** from backlog — codebase-wide `request: any` pattern; runtime catches typos; not pursuing |
| RC-20 | Minor | T10 quality-reviewer | Test #6 baseline preservation only checks 2 endpoints (`/api/identity/me`, `/api/location/type`); name overpromises | C (1-line) | **FIXED** at Phase 4.1 4.1-T5 (renamed to accurately describe 2-endpoint smoke scope) |
| RC-21 | Minor | T10 quality-reviewer | Login called 6× per test instead of `beforeAll` cookie reuse | F (defensible) | **DELETED** from backlog — Playwright best-practices favor per-test login isolation; matches Phase 3 pattern; not pursuing |
| RC-22 | Important | T12 done-gate / this retro | T12 plan defects: Step 1 destructive `down -v` + `prepareDocker` plugin bug + Step 9 `docker stats --filter` invalid flag | A (codify) | **FIXED** at Phase 4.1 4.1-T1 — `docs/process/plan-template-defects.md` documents all 3 defects (`down -v` destructive, `prepareDocker` plugin bug, `docker stats --filter` invalid flag) |
