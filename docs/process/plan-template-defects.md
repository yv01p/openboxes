...
## Cutover/seam-flip premise: verify the producer contract end-to-end before assuming upstream tasks left it complete (Phase 5.5 RC-52)

A strangler "cutover" task (flip the SPA/nginx seam from the legacy service to the new one) is routinely planned as frontend-only wiring on top of a write-cluster that earlier per-entity tasks are ASSUMED to have completed. That assumption is load-bearing and frequently false: upstream tasks may have shipped create-only (no upsert), unlinked default associations, list DTOs missing columns the legacy list served, or read endpoints the SPA never exercises. A cutover plan that doesn't re-verify the full producer contract (the round-trip the consumer actually drives) smuggles those gaps into production.

**Fix** (at plan-write time): a cutover/seam-flip task's FIRST step must be a real-payload ground-truth trace of the COMPLETE consumer round-trip (load → save → reload → list → edit) against the new service, BEFORE any wiring. Scope the task to OWN fixing whatever that trace reveals — even backend gaps nominally "closed" by upstream tasks. Do not write the cutover plan as if the upstream write-cluster is complete; write it as a verification-and-remediation task. When the premise turns out false, re-spec it (a falsified load-bearing assumption is a design defect), don't patch the cutover task ad hoc.

**Rationale**: Phase 5.5 CUT was planned as "switch the React seam + nginx; T2–T5 write-cluster is done." The ground-truth trace found three backend gaps the plan assumed didn't exist (LQ2 list-columns; PKG-FIX package upsert + default-link; an attribute read-filter + Jackson-strict payload mismatch). Rather than patch CUT, Phase 5.5 wrote a dedicated write-contract-reconciliation design + plan. See `synthetic-payload-blind-spot.md` § "Cutover is a verification task, not wiring" (RC-44).
